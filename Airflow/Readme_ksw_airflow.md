...
KAFKA_TOPIC = 'car_info'
API 키 관리
API 키는 Airflow 변수로 관리합니다. 다음 Airflow 변수를 설정하세요:

tdata_apikey: TDATA API 키
kakao_rest_api_key: Kakao API 키
Airflow CLI를 사용해 변수를 설정할 수 있습니다:
...
### 결론
대규모 API 호출을 처리할 때는 aiohttp와 asyncio를 활용한 비동기 처리가 동기 처리에 비해 훨씬 효율적입니다. 비동기 처리를 통해 전체 처리 시간을 크게 줄일 수 있으며, 성능을 향상시킬 수 있습니다.
